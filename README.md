...
The UI includes live download progress (parsed from yt-dlp output), preview
playback of the downloaded file through the plugin output (play/pause, drag
the bar under the waveform to scrub, playhead on the waveform), and the
Stash — a drawer listing your last 50 downloads (persisted to
`%APPDATA%/StashTrack/history.json`); click an entry to reload it for
preview and drag-to-DAW. `Tools/UiProbe.cpp` drives the real UI bundle
offscreen with synthetic input for verification
(`StashTrackUiProbe <bundle> <outDir>` writes PNG snapshots).
...
After rebuilding `dist/StashTrackv0.8.1Setup.exe`, upload it to GitHub Releases
both as `StashTrackv0.8.1Setup.exe` and as `StashTrackSetup.exe`. Future installer
...
```text
dist/StashTrackv0.8.1Setup.exe
```

`StashTrackv0.8.1Setup.exe` installs to the standard system VST3 folder:
